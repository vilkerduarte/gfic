  ...
  <div class="px-3 md:px-12 py-8 space-y-10">
    ...
    <Dialog v-model:visible="detailVisible" header="Detalhes do Ativo" :style="{ width: '840px' }">
      <div v-if="detail" class="space-y-6">
        ...
        <section v-if="detailLoading" class="space-y-4">
          <p class="text-sm text-slate-400">Carregando dados detalhados...</p>
          <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <Skeleton v-for="idx in 4" :key="`metric-skeleton-${idx}`" height="72px" borderRadius="12px" />
          </div>
          <Skeleton height="180px" borderRadius="14px" />
          <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            <Skeleton v-for="idx in 2" :key="`candle-skeleton-${idx}`" height="110px" borderRadius="12px" />
          </div>
        </section>

        <section v-else-if="detailError" class="rounded-xl border border-amber-700 bg-amber-950/30 p-4 text-sm text-amber-200">
          <p class="font-semibold">Nao foi possivel carregar os detalhes completos.</p>
          <p class="mt-1 text-amber-100">{{ detailError }}</p>
        </section>
...
const buildPreviewDetailData = (asset = {}) => ({
  stock: {
    ...asset,
    close: asset?.price ?? asset?.close ?? null,
    change: asset?.change ?? null,
    changePercent: asset?.changePercent ?? null
  }
})

const mergeDetailData = (asset, data = {}) => ({
  ...data,
  stock: {
    ...buildPreviewDetailData(asset).stock,
    ...(data?.stock || {})
  }
})
...
const handleDetails = async (asset) => {
  const currentRequest = ++detailRequestVersion
  detailVisible.value = true
  detailLoading.value = true
  detailError.value = ''
  detailData.value = buildPreviewDetailData(asset)

  try {
    let data = null
    const socket = getSocket()
    if (auth?.isAuthenticated.value && socket && socket.connected) {
      data = await emitAsync(
        'asset:details',
        { symbol: asset.symbol },
        { timeoutMs: 60000 }
      )
    ...
    if (currentRequest !== detailRequestVersion) return
    detailData.value = mergeDetailData(asset, data)
  } catch (err) {
    if (currentRequest !== detailRequestVersion) return
    detailError.value =
      err?.message === 'timeout'
        ? 'Tempo limite excedido ao buscar detalhes do ativo.'
        : err?.message || 'Falha ao buscar detalhes'
  } finally {
    if (currentRequest === detailRequestVersion) {
